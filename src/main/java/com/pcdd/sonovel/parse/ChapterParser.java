    ...
    private String resolveNextUrl(Document doc, Elements nextEls, Rule.Chapter r) {
        try {
            ...
            if (r.getNextPageInJs() != null) {
                String nextUrl = JsoupUtils.selectAndInvokeJs(doc, r.getNextPageInJs(), ContentType.HTML);
            ...
            // 从按钮获取下一页链接
            Element firstEl = nextEls.first();
            if (firstEl == null) {
                Console.error("无法获取下一页按钮元素\n出错链接：{}", doc.baseUri());
                return null;
            }
            
            return firstEl.absUrl("href");
        } catch (Exception e) {
            Console.error("解析下一页链接时出错：{}\n出错链接：{}", e.getMessage(), doc.baseUri());
            return null;
        }
    }
